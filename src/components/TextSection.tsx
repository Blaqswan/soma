...
export default function TextSection({
  ...
  // Title Style Resolution (changed default alignment from right to left)
  ...
  // Body Style Resolution (changed default alignment from right to left)
  ...
  // Align flex items based on the text alignment (default to left alignment for left sidebar)
  const alignment = titleStyle.textAlign || bodyStyle.textAlign || "left";
  const alignClass =
    alignment === "left"
  ...
  return (
    <div
      onClick={onClick}
      className={`
        relative w-full flex-1 flex flex-col group/section select-none cursor-pointer p-3.5 rounded-xl border border-transparent transition-all duration-300
        ${alignClass}
        ${hasBackground
      ...
      <div className="absolute top-2 left-2 opacity-0 group-hover/section:opacity-100 transition-opacity duration-200 z-10 pointer-events-none">
        <div className={`
          ...
          ${hasBackground
            ...
            : "bg-white/90 dark:bg-zinc-900/90 text-zinc-650 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-800"
          }
        `}>
        ...
        <>
          {titleText && (
            <h4
              style={titleCSS}
              className={`
                uppercase leading-tight break-words w-full transition-all duration-300
                ${bodyText || link ? "mb-1" : ""}
          ...
          {link && (
            <div className={`
              ...
              ${hasBackground
                ...
                : "text-indigo-650 dark:text-indigo-400 hover:underline"
              }
            `}>
              <span>More Info</span>
